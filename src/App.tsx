  ...
  function parseCSVText(raw){
    try{
      ...
      metricCols.forEach(function(col){
        var vals=sourceRows.map(function(r){return r[col];}).filter(function(v){return typeof v==="number"&&!isNaN(v);});
        if(vals.length>0){
          var sum=vals.reduce(function(a,b){return a+b;},0);
          summary[col]=Math.round((vals.length>1?sum:vals[0])*100)/100;
        }
      });
      ...
      var dataStr="GA4 DATA"+(matchedPage?" for "+selectedPage:" (site-wide)")+":\n";
      dataStr+="Rows: "+rows.length+" | Columns: "+headers.join(", ")+"\n";
      dataStr+="Path column: "+(pathCol||"not detected")+"\n\n";
      Object.keys(summary).forEach(function(k){dataStr+=k+": "+summary[k]+"\n";});
      dataStr+="\nData sample:\n";
      sourceRows.slice(0,10).forEach(function(r){
...
export default function App(){
  ...
  useEffect(function(){return onAuthStateChanged(_auth,function(u){if(u){if(!u.email||!u.email.endsWith("@gwi.com")){fbSignOut(_auth);_setUser(null);_setLoginError("Access restricted to @gwi.com accounts.");_setAuthLoading(false);return;}_setUser(u);getDoc(doc(_db,"users",u.uid)).then(function(snap){if(snap.exists()){var d=snap.data();if(d.auditData)setAuditData(d.auditData);if(d.generatedAudits)setGeneratedAudits(d.generatedAudits);}});}else{_setUser(null);}_setAuthLoading(false);});},[]);
  useEffect(function(){if(!_user)return;var t=setTimeout(function(){setDoc(doc(_db,"users",_user.uid),{auditData:auditData,generatedAudits:generatedAudits,email:_user.email,ts:Date.now()},{merge:true});},2000);return function(){clearTimeout(t);};},[auditData,generatedAudits,_user]);
