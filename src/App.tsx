  ...
  return(
    ...
    <div style={{display:"flex",flexDirection:isMobile?"column":"row",flex:1,minHeight:0,background:C.grey2}}>
      <div style={{width:isMobile?"100%":220,background:C.white,borderRight:"1px solid "+C.grey4,flexShrink:0,display:"flex",flexDirection:"column",overflow:"auto"}}>
        ...
        {buildPicking&&<div style={{borderBottom:"1px solid "+C.grey4,background:C.grey2,padding:"6px 8px",maxHeight:260,overflowY:"auto"}}>{(auditData||[]).length===0?<div style={{fontSize:12,color:C.grey6,textAlign:"center",padding:"12px 0",fontStyle:"italic"}}>No recommendation pages yet</div>:(auditData||[]).map(function(page:any){var pc=_priCfg[page.priority]||_priCfg.Medium;return(<button key={page.id} onClick={function(){setBuildPage(page);setBuildPicking(false);}} style={{width:"100%",textAlign:"left",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",gap:8,marginBottom:2}} onMouseEnter={function(e){e.currentTarget.style.background=C.grey3;}} onMouseLeave={function(e){e.currentTarget.style.background="transparent";}}><span style={{background:pc.bg,color:pc.text,border:"1px solid "+pc.border,fontSize:9,fontWeight:700,padding:"1px 6px",borderRadius:99,flexShrink:0}}>{page.priority}</span><span style={{fontSize:12,fontWeight:600,color:C.offBlack,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{page.label}</span></button>);})}</div>}
        {(function(){
