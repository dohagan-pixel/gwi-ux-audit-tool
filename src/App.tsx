...
import { GlassLogoPage } from "./v2/GlassLogoPage";
import { MarketReportsPage } from "./v2/MarketReportsPage";
...
var cardStyle={background:C.white,border:"1.5px solid "+C.grey4,borderRadius:14,padding:20,textAlign:"left",cursor:"pointer",display:"flex",flexDirection:"column",gap:10,fontFamily:FF};
              ...
              <div
                ...
                <div style={{display:"flex",alignItems:"center",gap:14,padding:"16px 20px"}}>
                  ...
                  <button onClick={function(){setOpenPage(isOpen?null:page.id);}} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:12,textAlign:"left",padding:0,minWidth:0}}>
                    {(function(){var _ps=pages.find(function(p){return p.url===page.url;});var _label=(_ps&&_ps.label)||page.label;var _du=(_ps as any)?.displayUrl||page.url.replace(/^https?:\/\//,"").replace(/\/$/,"");return(<><span style={{fontWeight:700,color:C.black,fontSize:15,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{_label}</span>{!isMobile&&<span style={{fontFamily:"monospace",color:C.pink,fontSize:12,flexShrink:0}}>{_du}</span>}</>);})()}
...
var LOADING_WORDS=["Frolicking","Flibbertigibbeting","Schlepping","Spelunking","Noodling","Beboppin'","Marinating","Combobulating","Sussing","Wibbling","Jiving","Puttering","Coalescing","Elucidating","Cogitating","Ideating","Brewing","Reticulating","Transmuting","Mulling","Simmering","Germinating"];
function pickWord(prev:number){var next;do{next=Math.floor(Math.random()*LOADING_WORDS.length);}while(next===prev);return next;}
...
function GeneratingModal({onClose,onDone,prompt,pageLabel,images}){
  ...
  var [wordIdx,setWordIdx]=useState(function(){return Math.floor(Math.random()*LOADING_WORDS.length);});
  var [wordKey,setWordKey]=useState(0);
  useEffect(function(){var t=setInterval(function(){setWordIdx(pickWord);setWordKey(function(k){return k+1;});},4000);return function(){clearInterval(t);};},[]);
...
function WireframeModal({page,personas,onClose,onSave,rules}){
  ...
  useEffect(function(){
    ...
    fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:prompt,max_tokens:16000})})
      .then(function(r){
        ...
        function stripFences(t){t=t.replace(/^```[\w]*\n?/,"").replace(/\n?```\s*$/,"").trim();var idx=t.indexOf("<!DOCTYPE");if(idx<0)idx=t.indexOf("<html");if(idx>0)t=t.slice(idx);return t;}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}@keyframes wordFade{0%{opacity:0;transform:translateY(6px)}7%{opacity:1;transform:translateY(0)}85%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(-4px)}}@keyframes wordPulse{0%,100%{opacity:1}50%{opacity:0.35}}`}</style>
      <div style={{background:C.white,borderRadius:16,padding:32,width:440,maxWidth:"90vw",textAlign:"center"}}>
        {status==="loading"&&(<><div style={{width:48,height:48,borderRadius:"50%",border:"4px solid "+C.grey4,borderTop:"4px solid "+C.pink,margin:"0 auto 20px",animation:"spin 0.8s linear infinite"}}/><div style={{fontSize:11,fontWeight:700,color:C.grey6,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10}}>Generating wireframe</div><div style={{height:44,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",marginBottom:16}}><span key={wordKey} style={{fontSize:28,fontWeight:900,color:C.black,letterSpacing:"-0.02em",display:"inline-block",animation:"wordFade 4s ease-in-out forwards"}}><span style={{display:"inline-block",animation:"wordPulse 0.8s ease-in-out infinite"}}>{LOADING_WORDS[wordIdx]}</span></span></div><p style={{fontSize:13,color:C.grey7,margin:"0 0 6px"}}>Building a design concept for the improved {page.label} page…</p><p style={{fontSize:12,color:C.grey6,margin:"0 0 24px"}}>This usually takes around a minute — the more context, the sharper the output.</p><div style={{background:C.grey3,borderRadius:99,height:8,overflow:"hidden",marginBottom:8}}><div style={{width:progress+"%",background:C.pink,height:"100%",borderRadius:99,transition:"width 0.4s ease"}}/></div><div style={{fontSize:12,color:C.grey6,textAlign:"right",marginBottom:16}}>{Math.round(progress)}%</div><button onClick={function(){onClose(false);}} style={{background:"none",border:"1px solid rgb(16,23,32)",color:"rgb(16,23,32)",borderRadius:8,padding:"10px 20px",fontSize:13,fontWeight:600,cursor:"pointer",width:"100%"}}>Cancel</button></>)}
            ...
            return(
              <div key={folder.scope}
                ...
                {isOpen&&(<>
                  ...
                  {starred.map(function(a){var isActive=a.id===activeAudit;var _ps=pages.find(function(p:any){return p.url===a.scope;});var _al=(_ps&&_ps.label)||a.pageLabel;return(
                    <div key={a.id} style={{background:isActive?"#FFF0F7":"transparent"}}>
                      <button onClick={function(){setActiveAudit(a.id);setEditing(false);}} style={{width:"100%",textAlign:"left",padding:"8px 14px 8px 34px",border:"none",borderLeft:"3px solid "+(isActive?"#FFC107":"transparent"),background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:700,color:isActive?C.black:C.grey8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{_al}</div>
                  ...
                  {unstarred.map(function(a){var isActive=a.id===activeAudit;var _ps=pages.find(function(p:any){return p.url===a.scope;});var _al=(_ps&&_ps.label)||a.pageLabel;return(
                    <div key={a.id} style={{background:isActive?"#FFF0F7":"transparent"}}>
            ...
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                {editing?(
                  ...
                  <>
                    <h2 style={{color:C.white,fontSize:22,fontWeight:800,margin:"0 0 4px"}}>{(function(){var _ps=pages.find(function(p:any){return p.url===audit.scope;});return(_ps&&_ps.label)||audit.pageLabel;}())}</h2>
...
function SummaryPage({personas,stages,pages,journeys,onAuditGenerated,onViewGenerated,clientList,caseStudies,verticals}:{personas:any,stages:any,pages:any,journeys:any,onAuditGenerated:any,onViewGenerated:any,clientList:any,caseStudies:any,verticals:any}){
    ...
    <div style={{background:C.black,borderBottom:"1px solid "+C.offBlack,flexShrink:0,position:"relative",zIndex:50}}>
      ...
      {menuOpen&&(
        <div style={{background:C.black,borderTop:"1px solid "+C.offBlack,padding:"8px 8px 12px"}}>
          ...
          <Btn id="content-hub" label="Content Hub"/>
          <Btn id="screenshots" label="Screenshots"/>
...
function WireframesPage({wireframes,setWireframes,onDeleteWireframe,onUpdateWireframe,auditData,personas,onSaveWireframe,onAddRec,onRemoveRec,lovedComponents,onLoveComponent,onUnloveComponent,wireframeRules,pages}:any){
  ...
  function viewFullPage(){
    ...
    // Force margin/padding to 0 directly on body element (beats user-agent stylesheet)
    if(parsedDoc.body){parsedDoc.body.style.setProperty('margin','0','important');parsedDoc.body.style.setProperty('padding','0','important');}
                ...
                <div key={lc.id} style={{background:C.white,borderRadius:16,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",flexShrink:0}}>
                  <div style={{padding:"18px 24px",borderBottom:"1px solid "+C.grey4,display:"flex",alignItems:"flex-start",gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:11,color:C.grey6}}>{lc.date} · from {(function(){var _ps=(pages||[]).find(function(p:any){return p.url===lc.pageUrl;});return(_ps&&_ps.label)||lc.pageLabel;}())} wireframe</div>
                    </div>
            ...
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                {editing?(
                  ...
                  <>
                    <h2 onClick={openEdit} title="Click to rename" style={{color:C.white,fontSize:22,fontWeight:800,margin:"0 0 4px",cursor:"text",borderBottom:"1px dashed rgba(255,255,255,0.2)",display:"inline-block",paddingBottom:1}}>{active.pageLabel||((pages||[]).find(function(p:any){return p.url===active.pageUrl;}) as any)?.label||""}</h2>
                    <p style={{color:C.grey6,fontSize:13,margin:"4px 0 0"}}>{active.date} · wireframe</p>
                  </>
                )}
              </div>
              {!editing&&(
...
export default function App(){
  ...
  var VALID_VIEWS=["dashboard","audit","generated-audits","summary","personas","persona-detail","mapping","journey","lifecycle","affinity","flows","analytics","settings","wireframes","feedback","guide","landing","qa-walkthrough","content-hub","screenshots","content-plan","content-refresh","content-geo-seo","plan-vs-seo-geo","glass-logo","market-reports"];
...
getDocs(collection(_db,"users",u.uid,"feedback")).then(function(snap){var arr=snap.docs.map(function(d){return d.data();});setFeedback(function(prev){var merged=prev.slice();arr.forEach(function(a){if(!merged.find(function(x){return x.id===a.id;}))merged.push(a);});return merged;});}).catch(function(){});}else{_setUser(null);}_setAuthLoading(false);});},[]);
  ...
  var _NAV_VIEWS=["dashboard","summary","audit","wireframes","personas","mapping","analytics","qa-walkthrough","content-hub","screenshots"];
  var _NAV_MAP:Record<string,string>={dashboard:"dashboard",summary:"summary","generated-audits":"summary",wireframes:"wireframes",audit:"audit",personas:"personas","persona-detail":"personas",mapping:"mapping",journey:"mapping",lifecycle:"mapping",affinity:"mapping",flows:"mapping",analytics:"analytics","qa-walkthrough":"qa-walkthrough","content-hub":"content-hub",screenshots:"screenshots"};
  useEffect(function(){
  ...
  var _qaSharePath=window.location.pathname.match(/^\/share\/qa\/([a-z0-9]+)$/);
  if(_qaSharePath)return(<SharedQAReport shareId={_qaSharePath[1]}/>);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",fontFamily:FF}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",paddingTop:isMobile?0:56}}>
        ...
        {view==="glass-logo"&&<GlassLogoPage/>}
        {view==="market-reports"&&<MarketReportsPage/>}
