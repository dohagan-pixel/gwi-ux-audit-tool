...
function WireframesPage({wireframes,setWireframes,onDeleteWireframe,onUpdateWireframe,auditData,personas,onSaveWireframe,onAddRec,onRemoveRec,lovedComponents,onLoveComponent,onUnloveComponent}:any){
  ...
  return(
    <>{_buildInfoModal}{buildPage&&<WireframeModal page={buildPage} personas={personas||[]} onClose={function(){setBuildPage(null);}} onSave={function(wf){if(onSaveWireframe)onSaveWireframe(wf);setActiveId(wf.id);}}/>}
    <div style={{display:"flex",flexDirection:isMobile?"column":"row",flex:1,minHeight:0,background:C.grey2}}>
      <div style={{width:isMobile?"100%":220,background:C.white,borderRight:"1px solid "+C.grey4,flexShrink:0,display:"flex",flexDirection:"column",overflow:"auto"}}>
