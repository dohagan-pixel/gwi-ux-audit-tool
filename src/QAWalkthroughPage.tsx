...
export function buildHtml(meta: ExportMeta, answers: Answers): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>UX QA report — ${escapeHtml(meta.pageName || meta.url)}</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font:14px/1.55 'Faktum',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:${C.ink};background:#fff;padding:48px;max-width:1100px;margin:0 auto}h1{font-size:36px;letter-spacing:-0.02em;margin:6px 0 14px}.eye{font-size:11px;text-transform:uppercase;letter-spacing:0.12em;color:${C.pink};font-weight:700}.k{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:24px 0 32px}.k .b{background:${C.grey2};padding:16px;border-radius:12px}.k .v{font-size:28px;font-weight:800;letter-spacing:-0.02em}.k .l{font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:${C.grey7};margin-top:4px}</style></head><body><div class="eye">GWI · UX QA report</div><h1>${escapeHtml(meta.pageName || meta.url)}</h1><div style="font-size:13px;color:${C.grey7}"><a href="${escapeHtml(meta.url)}" style="color:${C.pink}">${escapeHtml(meta.url)}</a>${meta.reviewer ? ` · Reviewed by ${escapeHtml(meta.reviewer)}` : ""} · Finished ${escapeHtml(meta.finishedAt)}${meta.asanaLink ? ` · <a href="${escapeHtml(meta.asanaLink)}" style="color:${C.pink}">Asana</a>` : ""}</div><div class="k"><div class="b"><div class="v">${passPct}%</div><div class="l">Pass rate</div></div><div class="b"><div class="v" style="color:${C.pass}">${pass}</div><div class="l">Pass</div></div><div class="b"><div class="v" style="color:${C.fail}">${fail}</div><div class="l">Flag</div></div><div class="b"><div class="v" style="color:${C.na}">${na}</div><div class="l">N/A · skipped</div></div></div>${issuesBlock}${sectionsHtml}<footer style="margin-top:48px;padding-top:20px;border-top:1px solid ${C.grey4};font-size:12px;color:${C.grey7}">Generated by GWI UX QA · ${escapeHtml(meta.finishedAt)}</footer></body></html>`;
}
